    ...
    <div style={{ position: "relative", overflow: "hidden", ...style }} onClick={onClick}>
      {!loaded && !error && (
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center",
          justifyContent: "center", background: "#141210", color: "#3a3530", fontSize: 11 }}>
          Loading…
        </div>
      )}
      {error ? (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column",
          ...
          <span style={{ fontSize: 10, color: "#4a4540" }}>Image unavailable</span>
        </div>
      ) : (
        <img src={src} alt={alt || ""}
...
export default function JudgingApp() {
  ...
  const S = {
    ...
    heroSub:   { fontSize: "14px", color: "#6b6560", lineHeight: 1.7, maxWidth: 480, margin: "0 auto" },
    ...
    btnMuted:{ display: "block", width: "100%", marginTop: 10, padding: "10px", background: "transparent", border: "1px solid #2a2a2a", borderRadius: 5, color: "#5a5550", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", fontFamily: "'Georgia', serif", cursor: "pointer" },

    // Category grid
    catGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px,1fr))", gap: 10, maxWidth: 880, margin: "0 auto", padding: "0 24px" },
    catCard: (state) => ({
      background: state === "done" ? "#1a2518" : state === "noaward" ? "#1a1a1a" : "#181614",
      border: `1px solid ${state === "done" ? "#2d4a2d" : state === "noaward" ? "#2a2a2a" : "#2a2a2a"}`,
      borderRadius: 8, padding: "22px 18px", cursor: "pointer", transition: "all 0.2s", position: "relative",
    }),
    catName:     { fontSize: "15px", fontWeight: 400, color: "#e8e4df", marginBottom: 3 },
    catCount:    { fontSize: "11px", color: "#4a4540", letterSpacing: "0.5px" },
    catEssayTag: { display: "inline-block", fontSize: "8px", letterSpacing: "1px", textTransform: "uppercase", color: "#d4a017", border: "1px solid #3a2a00", borderRadius: 3, padding: "1px 5px", marginBottom: 4 },
    catBadgeDone:    { position: "absolute", top: 10, right: 12, fontSize: "10px", color: "#6aaa6a", letterSpacing: "1px" },
    catBadgeNoAward: { position: "absolute", top: 10, right: 12, fontSize: "10px", color: "#5a5550", letterSpacing: "1px" },
    ...
    judgeWrap: { maxWidth: 880, margin: "0 auto", padding: "18px 24px 180px" },
    ...
    // Single-image cards
    card:    (p) => ({ background: "#181614", border: `1px solid ${p ? PLACE_COLORS[p].border : "#2a2a2a"}`, borderRadius: 8, overflow: "hidden", marginBottom: 14, transition: "border-color 0.2s" }),
    imgWrap: { width: "100%", aspectRatio: "3/2", background: "#141210", position: "relative", cursor: "zoom-in", overflow: "hidden" },
    badge:   (p) => ({ position: "absolute", top: 8, left: 8, zIndex: 2, background: PLACE_COLORS[p].bg, color: PLACE_COLORS[p].text, fontSize: "9px", fontWeight: 700, letterSpacing: "1.5px", padding: "3px 8px", borderRadius: 3 }),
    entryInfo:    { padding: "14px 18px 16px" },
    entryHeadline:{ fontSize: "15px", fontWeight: 400, color: "#e8e4df", marginBottom: 5 },
    entryCaption: { fontSize: "14px", color: "#c8c4bf", lineHeight: 1.65 },
    voteRow:  { display: "flex", gap: 7, marginTop: 14, flexWrap: "wrap" },
    voteBtn:  (place, active, disabled) => ({
      ...
      color: active ? PLACE_COLORS[place].text : disabled ? "#2a2520" : "#6a6560",
    ...
    // Essay folder grid
    essayGrid:   { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px,1fr))", gap: 14 },
    ...
    folderOverlay:{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.65) 0%,transparent 55%)", zIndex: 1, display: "flex", alignItems: "flex-end", padding: "10px 12px" },
    folderCount: { fontSize: "10px", color: "#e8e4df", background: "rgba(0,0,0,0.55)", padding: "2px 8px", borderRadius: 10 },
    folderInfo:  { padding: "12px 14px 14px" },
    folderTitle: { fontSize: "14px", fontWeight: 400, color: "#e8e4df", marginBottom: 10, lineHeight: 1.3 },
    viewBtn:     { background: "none", border: "1px solid #2a2a2a", borderRadius: 4, color: "#7a7570", fontSize: "10px", letterSpacing: "1px", padding: "5px 10px", cursor: "pointer", fontFamily: "'Georgia', serif", transition: "all 0.18s", display: "block", marginBottom: 10 },

    // Essay detail
    essayDetailTitle: { fontSize: "clamp(18px,2.5vw,24px)", fontWeight: 400, color: "#e8e4df", marginBottom: 4 },
    essayDetailMeta:  { fontSize: "12px", color: "#6b6560", marginBottom: 28 },
    essayPhotoGrid:   { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px,1fr))", gap: 12, marginBottom: 40 },
    ...
    essayPhotoInfo:   { padding: "10px 12px 12px" },
    essayPhotoNum:    { fontSize: "9px", letterSpacing: "1.5px", textTransform: "uppercase", color: "#4a4540", marginBottom: 4 },
    essayPhotoCap:    { fontSize: "13px", color: "#c8c4bf", lineHeight: 1.55 },

    // Comment box
    commentBox:   { marginTop: 24, padding: "16px", background: "#1a1816", border: "1px solid #2d4a2d", borderRadius: 6 },
    commentLabel: { fontSize: "11px", letterSpacing: "1.5px", textTransform: "uppercase", color: "#6aaa6a", marginBottom: 8, display: "block" },

    // Submit bar
    submitBar:   { position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(15,15,15,0.96)", backdropFilter: "blur(12px)", borderTop: "1px solid #2a2a2a", padding: "14px 24px", zIndex: 40 },
    submitInner: { maxWidth: 880, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" },
    submitStatus:{ fontSize: "11px", color: "#5a5550", letterSpacing: "0.5px", lineHeight: 1.7 },
    submitHint:  { fontSize: "10px", color: "#3a3530", letterSpacing: "0.5px" },
    submitWarn:  { fontSize: "11px", color: "#c0504d", fontStyle: "italic" },
    submitBtns:  { display: "flex", gap: 10, alignItems: "center", flexShrink: 0 },
    skipBtn:     { padding: "9px 16px", background: "transparent", border: "1px solid #2a2a2a", borderRadius: 5, color: "#5a5550", fontSize: "10px", letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "'Georgia', serif", cursor: "pointer", transition: "all 0.18s" },
    submitOn:    { padding: "10px 24px", background: "#d4a017", border: "none", borderRadius: 5, color: "#1a1a1a", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", fontFamily: "'Georgia', serif", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" },
    submitOff:   { padding: "10px 24px", background: "#1e1c18", border: "1px solid #2a2a2a", borderRadius: 5, color: "#3a3530", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", fontFamily: "'Georgia', serif", fontWeight: 700, cursor: "not-allowed", whiteSpace: "nowrap" },
    ...
    successSub:   { fontSize: "14px", color: "#6b6560", lineHeight: 1.7, maxWidth: 440, margin: "0 auto" },
    ...
    lbCaption: { marginTop: 14, fontSize: "14px", color: "#c8c4bf", maxWidth: 680, textAlign: "center", lineHeight: 1.6 },
    lbClose:   { position: "absolute", top: 18, right: 24, background: "none", border: "none", color: "#6b6560", fontSize: "24px", cursor: "pointer", lineHeight: 1, zIndex: 1 },

    // Admin panel
    adminGrid:   { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px,1fr))", gap: 8, maxWidth: 880, margin: "0 auto", padding: "0 24px 60px" },
    adminCard:   { background: "#181614", border: "1px solid #2a2a2a", borderRadius: 8, padding: "16px" },
    adminName:   { fontSize: "13px", color: "#e8e4df", marginBottom: 8, letterSpacing: "0.5px" },
    adminCatRow: { fontSize: "11px", color: "#5a5550", lineHeight: 1.8 },
    adminDone:   { color: "#6aaa6a" },
    adminPending:{ color: "#3a3530" },
  };
        ...
        <div style={S.submitInner}>
          ...
          <div style={S.submitBtns}>
            <button style={S.skipBtn}
              onMouseEnter={(e) => { e.target.style.borderColor="#5a5550"; e.target.style.color="#8a8580"; }}
              onMouseLeave={(e) => { e.target.style.borderColor="#2a2a2a"; e.target.style.color="#5a5550"; }}
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                ...
                {cat.isEssayCategory && <div style={S.catEssayTag}>Essay</div>}
  ...
  if (phase === "judge" && selectedCat) {
    ...
    // ── Essay: detail view (individual photos) ─────────────────
    if (selectedCat.isEssayCategory && viewingEssay) return (
      <div style={S.app}><div style={S.grain} />
        <Header right={`Judging as: ${judgeId}`} />
        <Lightbox />
        <div style={S.backNav}>
            ...
            {viewingEssay.photos.map((photo) => (
              <div key={photo.id} style={S.essayPhotoCard}>
                ...
                <div style={S.essayPhotoInfo}>
                  ...
                  {photo.headline && <div style={{ ...S.essayPhotoCap, color: "#e8e4df", marginBottom: 4 }}>{photo.headline}</div>}
    ...
    if (selectedCat.isEssayCategory) return (
      <div style={S.app}><div style={S.grain} />
        ...
        <div style={S.judgeWrap}>
          ...
          <div style={S.catMeta}>
            {selectedCat.entries.length} entries · Click an entry to view all images · All placements optional · Up to {MAX_HMS} HMs
          </div>
                ...
                <div key={essay.id} style={S.folderCard(myPlace)}>
                  ...
                  <div style={S.folderInfo}>
                    ...
                    <button style={S.viewBtn}
                      ...
                      onMouseLeave={(e) => { e.target.style.borderColor="#2a2a2a"; e.target.style.color="#7a7570"; }}
                      onClick={() => setViewingEssay(essay)}>
                      View {essay.imageCount} images →
                    </button>
              ...
              <div key={entry.id} style={S.card(myPlace)}>
                <div style={S.imgWrap}
                  ...
                  <ContestImage src={entry.imageUrl} alt={entry.caption || entry.filename}
                    style={{ width: "100%", height: "100%" }} />
                  {myPlace && <span style={S.badge(myPlace)}>{PLACE_LABELS[myPlace].toUpperCase()}</span>}
                  <span style={{ position: "absolute", bottom: 8, right: 10, zIndex: 1, fontSize: "9px", color: "#ccc", background: "rgba(0,0,0,0.55)", padding: "2px 7px", borderRadius: 3, pointerEvents: "none" }}>
                    Click to enlarge
                  </span>
                </div>
